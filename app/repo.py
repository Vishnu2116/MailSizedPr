...
def update_job_email(db: Session, upload_id: str, email: str):
    """Update user email for a job before queueing."""
    job = db.query(Job).filter(Job.upload_id == upload_id).first()
    if not job:
        return None
    job.email = email
    db.commit()
